...
const ace = window.ace; // import { Range, EditSession, UndoManager } from "ace-builds";
...
const TokenIterator$2 = ace.require("ace/token_iterator").TokenIterator; // console.log(TextMode);
...
/* jslint esversion:8 */
const TextHighlightRules$1 = TextHighlightRules$2; // var rules = null;
...
class ResourceMode extends TextMode {
  constructor(editor, onlyHighlight = false) {
    ...
    this.rules = new Rules(this.editor.session.language || "en", this.$id); // If static highlighter, do not watch for changes
...
class CodeEditor {
  ...
  setHighlightRule(editor, fileName) {
    ...
    const ext = fileName ? fileName.substring(fileName.indexOf(".")) : ".kbf";
    ...
    if (ext === ".kbf") {
      ...
      const modePath = autoImplementedMode(fileName);
